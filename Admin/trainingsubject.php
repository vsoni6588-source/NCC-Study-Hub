...
if(isset($_POST['add'])){
    ...
    mysqli_query($conn, "INSERT INTO subjects (title, description, type)
            VALUES ('$title', '$description', 'training')");
    ...
    header("Location: layout.php?page=trainingsubject");
    exit;
}
...
if(isset($_POST['update'])){
    ...
    if(isset($_FILES['pdf']) && $_FILES['pdf']['error'] == 0){
        ...
        if(move_uploaded_file($tmp_name, $file_path)){
            ...
            // 3. CHECK OLD FILE
            $check = mysqli_query($conn, "SELECT * FROM resources WHERE subject_id=$id");
            ...
            echo "File upload failed during update!";
        }
    }
...
<head>
<meta charset="UTF-8">
<title>Manage Training Subjects</title>
...
<h2>Manage Training Subjects</h2>
...
<tr>
    ...
    <td>
        <?php echo $row['file_name'] ? $row['file_name'] : "No File"; ?>
    </td>

    <td>
        <a class="edit" href="layout.php?page=trainingsubject&edit=<?php echo $row['id']; ?>">Edit</a>
    </td>
    ...
    <td>
        <a class="delete" href="layout.php?page=trainingsubject&delete=<?php echo $row['id']; ?>" 
